...
//
    ...
    pub fn add_dependency(&mut self, name: &str, version: &str) {
        ...
        let key = self.0.new_key(name);
        let val = self.0.new_value(version);
        let dep = self.0.new_entry(iter::once(key), val);
...
fn find_table(doc: &TomlDoc, ast: ast::Doc, keys: &[&str]) -> Option<ast::Table> {
    ast.tables(doc).find(|t| compare_keys(doc, t.header(doc), keys))
}
